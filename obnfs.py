...
try:
	...
	lvcreate=commands.getstatusoutput('sudo lvcreate -V'+d_size+'M --name '+d_name+' --thin blockstorage/blockpool')
	#The drive is formatted with xfs	
	lvformat=commands.getstatusoutput('sudo mkfs.xfs /dev/blockstorage/'+d_name)
	...
	os.system('sudo mount /dev/blockstorage/'+d_name+' /mnt/'+d_name)
	...
	make_sh='sudo echo mount 192.168.122.163:/mnt/{}   /media/{}>> obclientlinux.sh'.format(d_name,d_name)
	...
	print "<META HTTP-EQUIV='refresh' content='0; url=/obnfs.html'/>"
